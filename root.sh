...
# 函数：配置 root 用户 SSH 密钥认证 (已修改并修复语法错误)
ssh_key() {
    # 检查 root 权限
    if [[ $EUID -ne 0 ]]; then
...
# 函数：修改 root 密码 (修复：使用 read 明文显示输入, 增加 sudo 逻辑)
root_pwd() {
...
# ============================================
# 注册 RHEL 系统 (修复：使用 read 明文显示输入, 增加 sudo 逻辑)
# ============================================
register_rhel_system() {
    ...
    if command -v rhc >/dev/null 2>&1; then
        ...
        # 注意：rhc connect 可能不支持直接在命令行中输入密码，这里假设它支持
